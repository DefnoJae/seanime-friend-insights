...
function init(): void {
    $ui.register((ctx) => {
        ctx.dom.onReady(() => {
            // We inject a script tag directly into the browser context to bypass Goja runtime isolation
            const clientScript = `
                (function() {
                    const FRIEND_QUERY = \`
                    query ($mediaId: Int) {
                    ...
                    async function checkPage() {
                        ...
                        if (detectedId && detectedId !== currentMediaId) {
                            ...
                            // Targets Seanime's media page header button row or title block container
                    ...
                    // Poll browser frame location changes seamlessly 
                    setInterval(checkPage, 1500);
                })();
            `;

            // Execute code string directly on the browser document layout assembly
